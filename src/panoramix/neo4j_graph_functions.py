...
OPTIONAL MATCH (s:SpecificationNode) WHERE NOT (s)<-[:IS_SPECIFIED_BY]-(:ValueNode) AND NOT (s)<-[:ANNOTATES]-(:AnnotationNode) WITH collect(s) AS value_for_spec, n_types
...
OPTIONAL MATCH (n:SpecificationNode) WHERE NOT (n)<-[:CONTAINS]-() WITH count(n) AS unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types
OPTIONAL MATCH (n) WHERE NOT (n)--() OR (n)--(n) WITH collect(n) AS node_alone, unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types
OPTIONAL MATCH (n)-[:CONTAINS]->() WHERE (n:ValueNode AND n.value <> NULL) OR (n:SpecificationNode AND NOT n.type IN ['dict', 'list']) WITH collect(n) AS node_structure, node_alone, unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types
OPTIONAL MATCH (m)<-[:IS_SPECIFIED_BY]-(n:ValueNode)-[:IS_SPECIFIED_BY]->(o) WITH collect(n) as unique_spec, node_structure, node_alone, unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types
OPTIONAL MATCH (m)-[:CONTAINS]->(n:SpecificationNode)<-[:CONTAINS]-(o) WITH collect(n) as unique_s_parent, unique_spec, node_structure, node_alone, unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types
OPTIONAL MATCH (n:ValueNode)-[:IS_SPECIFIED_BY]->(s:SpecificationNode) WHERE (s.type IN ['dict', 'list'] AND NOT n.value IS :: NULL) OR (s.type = 'bool' AND NOT n.value IS :: BOOLEAN) or (s.type = 'int' AND NOT n.value IS :: INTEGER) or (s.type = 'float' AND NOT n.value IS :: FLOAT) or (s.type = 'str' AND NOT n.value IS :: STRING) WITH collect(n) as type_spec, unique_s_parent, unique_spec, node_structure, node_alone, unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types
OPTIONAL MATCH (vn:ValueNode) WHERE (vn)<-[:CONTAINS]-() AND NOT (vn)-[:IS_SPECIFIED_BY]->(:SpecificationNode)<-[:CONTAINS]-(:SpecificationNode)<-[:IS_SPECIFIED_BY]-(:ValueNode)-[:CONTAINS]->(vn) WITH collect(vn) AS prop_5, type_spec, unique_s_parent, unique_spec, node_structure, node_alone, unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types
OPTIONAL MATCH (n)-[]->(m), cyclePath=shortestPath((m)-[*]->(n)) WITH collect(cyclePath) as is_cycle, prop_5, type_spec, unique_s_parent, unique_spec, node_structure, node_alone, unique_root, duplicate, e_specifies, e_contains, option_annotation, file_annotation, value_for_spec, n_types

WITH *, [["There must not be cycles", is_cycle], ["For a non-root value node, the specification node of its mother must be the mother of its specification node ", prop_5], ["value nodes must be of the type of their specification", type_spec], ["A specification node must only have 1 parent specification node", unique_s_parent], ["A value node must be specified by only one specification node", unique_spec], ["a non-leaf node must have it value being NULL and be a value node or its type being 'dict' or 'list' and be a specification node", node_structure], ["A node must be connected to another one", node_alone], ["there must be exactly one specification root", unique_root], ["Nodes must not be duplicated", duplicate], ["specification edges must be from a value node to a Specification node", e_specifies], ["A containment edge must be between two nodes of the same type", e_contains], ["An annotation must be on a specification node or be a file node", option_annotation], ["A file node must be on a root value node", file_annotation], ["A specification node must either be annotated or specifiy a value node", value_for_spec], ["A node must have a label in ValueNode, SpecificationNode, AnnotationNode, FileNode", n_types]] as results
WITH *, [is_cycle = [], prop_5 = [], type_spec = [], unique_s_parent = [], unique_spec = [], node_structure = [], node_alone = [], unique_root = 1, duplicate = [], e_specifies = [], e_contains = [], option_annotation = [], file_annotation = [], value_for_spec = [], n_types = []] AS tests
WITH *, all(elmnt in tests WHERE elmnt = TRUE) as is_db_valid

RETURN is_db_valid,
CASE is_db_valid
  WHEN FALSE THEN [i IN range(0, size(tests)-1) WHERE NOT tests[i] | results[i]]
END AS info"""
