...
def v_node_creation_query(node):
    return f"CREATE ({STARTING_CHAR}{node.get_identifier()}:ValueNode {{value: {node.val()}}})"
    #                 ^^^^^^^^^^^^^ -- to make it so the identifier does not start with a number, neo4j does not like it

def s_node_creation_query(node):
    return f"CREATE ({STARTING_CHAR}{node.get_identifier()}:SpecificationNode {{name: {node.name()}, type: {node.stype()}}})"

def edge_creation_query(edge, relation):
    return f"({STARTING_CHAR}{edge.source().get_identifier()})-[{relation}]->({STARTING_CHAR}{edge.target().get_identifier()})"
